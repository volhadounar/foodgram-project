...
function Ingredients() {
    ...
    const addIngredient = (e) => {
        if(nameIngredient.value && cantidad.value) {
            ...
            elem.innerHTML = `<span> ${data.name} ${data.value}${data.units}</span> <span class="form__field-item-delete"></span>
                             <input id="id_ingredients-${cur}-recipe" name="ingredients-${cur}-ingredient" type="hidden" value="${data.name}">
                             <input id="id_ingredients-${cur}-recipe" name="ingredients-${cur}-amount" type="hidden" value="${data.value}">
                             <input id="id_ingredients-${cur}-recipe" name="ingredients-${cur}-unit" type="hidden" value="${data.units}">`;
            cur++;
    ...
    const getValue = (e) => {
        const data = {
            ...
            units: cantidadVal.value
        };
        clearValue(nameIngredient);
        clearValue(cantidad);
        clearValue(cantidadVal);
        return data;
    };
    // очистка инпута
